...
SmartShot's Manifest V3 WebExtension identifies semantic content blocks in Chrome and Safari. Everything stays on the Mac: the extension makes no network requests and sends only the completed PNG plus bounded technical metadata to SmartShot when its native-messaging bridge is available. The selector is injected only after you invoke the toolbar action or its browser shortcut; it does not run persistently across every website.
...
2. Hover a block. X posts (`article[data-testid="tweet"]`) are selected as one post. Media, social context, and nested quoted-post content resolve to the outer post article; sibling replies remain separate and threads are not combined. Other pages default to the nearest semantic `article`, list item, `main`, `section`, or supported ARIA ancestor when one exists; smaller layout blocks remain available through the level controls.
...
5. Blocks already inside the viewport use one fast browser capture. Taller or partially offscreen blocks use bounded scrolling capture: SmartShot scrolls through the selected DOM element, captures each visible slice twice at least 550 ms apart, rejects materially changed pixels, crops using the observed image-to-viewport scale, and stitches one PNG for local delivery. All browser capture calls share the same pacing queue, including visible captures and verification frames.
...
The Chromium extension is currently distributed for **Load unpacked** installation; it is not yet distributed through a browser store. Google Chrome, Chromium, Microsoft Edge, and Brave are supported by the native-host installer.
...
1. Build a complete SmartShot app with the Safari extension target embedded. Use Apple development signing when available. Without a development team, use Xcode's **Sign to Run Locally** for both the containing app and extension, and launch that exact containing app once. A custom persistent self-signed identity can run the native app but is not accepted by Safari as the local WebExtension development signature.
2. For a **Sign to Run Locally** development build, enable **Safari > Settings > Developer > Allow unsigned extensions** and complete Safari's authentication prompt. Safari resets this development setting whenever it quits, so it must be enabled again after a Safari restart. A properly Apple-signed distribution build should not require this setting.
...
4. Return to SmartShot and click **Refresh** to confirm the enabled state. Add or pin the SmartShot action in Safari's toolbar if it is not visible. The default browser-action shortcut is `Control-Shift-9` and can be changed in Safari's extension settings.

Do not replace an already registered development extension's resources in place. Safari can retain the old service worker registration while blanking the extension details. Build to a fresh path/version, launch that containing app, then disable and re-enable the extension. A version change can also remove the old toolbar item; add the new **Select a content block** item through **View > Customize Toolbar**.
...
- **Safari reports installed but disabled and does not list the local extension**: confirm the extension and containing app were signed together with Apple development signing or **Sign to Run Locally**, launch that exact containing app, enable **Allow unsigned extensions** for the current Safari process when using the latter, then reopen Safari's Extensions settings. A custom persistent self-signed app is insufficient for this Safari development path.
...
The shared geometry, selection fixtures, capture guards, content lifecycle, delivery fallback, and protocol tests have no external dependencies. Fixtures cover X timeline/detail posts with media, nested quotes, replies, and repost context, plus generic semantic blocks. They also verify fail-closed nested scrolling/sticky behavior, long-slice pixel-change detection, Promise/callback action reinjection and diagnostics, selector upgrade compatibility, strict capture response correlation, capture cancellation, per-tab action serialization and cross-tab independence, semantic defaults for pointer and accessibility clicks, rejection of non-semantic roles, background-owned fallback after page teardown, in-flight Escape/page-hide cancellation, the browser-action shortcut declaration, and page/style/scroll restoration. They do not replace a signed real-browser run against current X. The current-checkout run on 2026-08-24 passed 93/93 tests.
